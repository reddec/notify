...
#define __NOTIFY_MCAST_ADDR 3774347255 // 224.247.247.247
...
// Send notification with raw data. Wraps it to `notify_packet_t` and sends
// @param `socket_fd` - UDP socket descriptor
...
// Send notification with textual data. Wraps it to `notify_packet_t` and sends.
...
// Send notification of TCP service. Wraps it to `notify_packet_t` with format `notify_format_tcp_service` and sends.
// @param `socket_fd`    - UDP socket descriptor
// @param `service_name` - TCP logical service name
// @param `port`         - TCP listening service port
// @returns - same as `notify_packet`
ssize_t notify_local_tcp_service(int socket_fd, const char *service_name, uint16_t port);

// Send notification of UDP service. Wraps it to `notify_packet_t` with format `notify_format_udp_service` and sends.
// @param `socket_fd`    - UDP socket descriptor
...
// Parse notification message to `notify_packet_t` structure.
